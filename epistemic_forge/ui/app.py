...
with st.sidebar:
    ...
    model_choice = st.text_input(
        "Target Model",
        value="gpt-4o-mini",
        help="e.g., 'openai/gpt-4o', 'anthropic/claude-3-5-sonnet', 'ollama/llama3'",
    )
    api_base = st.text_input(
        "Custom API Base (Optional)", help="For local deployments via vLLM or Ollama."
    )

    st.header("Project Configuration")
    domain = st.selectbox(
        "Reasoning Domain", ["hybrid", "research", "philosophy", "kaggle", "freelance"]
    )
...
question = st.text_area(
    ...
    placeholder="If the brain is a prediction machine, what happens to moral responsibility?",
)
