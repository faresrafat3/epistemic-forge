...
def explore(spec: ProjectSpec, bundle: Dict[str, Any], beam: int = 3, steps: int = 2) -> SearchResult:
    ...
    for step in range(steps):
        ...
        for thought in proposed_thoughts:
            ...
            node = SearchNode(id=str(uuid.uuid4())[:8], thought=thought, value=score, meta={"step": step})
            nodes.append(node)
    ...
    return SearchResult(best_thought=best_thought_overall, nodes=nodes, mode_used="true_llm_tot", score=highest_score)
